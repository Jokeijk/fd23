...
int helper_pmlparameter(int npml,double pml_pos,double startpos,int nx,double *b,double*c,double*k,double dt,double R,double v,double fc)
{
  double *sg_u =new double[nx];
  double *kp_u =new double[nx];
  double *al_u =new double[nx];
  double *b_u  =new double[nx];
  double *c_u  =new double[nx];
  double *d    =new double[nx];

  const double pk=2.0;
  const double psig=2.0;
  const double palpha=1.0;

  double sigmax=-(psig+1.0)/(2.0*floor(pml_pos))*v*log(R);
  double alphamax=3.141592653589*fc;
  double kmax=2.0;
  ...
  for(int i=0;i<npml+2;i++){
	double tmp=( pml_pos-(i+startpos) )/floor(npml);
	if(tmp>0)	  d[i]=tmp;
  }
  for(int i=nx-npml-2;i<nx;i++){
	double tmp=( (i+startpos)-(nx-1-pml_pos) )/floor(npml);
	if(tmp>0)	  d[i]=tmp;
  }
...
//  safecall(cudaMemcpy(b, b_u,sizeof(double)*nx,cudaMemcpyHostToDevice));
//  safecall(cudaMemcpy(c, c_u,sizeof(double)*nx,cudaMemcpyHostToDevice));
//  safecall(cudaMemcpy(k,kp_u,sizeof(double)*nx,cudaMemcpyHostToDevice));
...
CPML::CPML(PARAM &param)
{
  ...
  int nx=param.nx;
  int nz=param.nz;
  double pml_dt=param.pml_dt;
  double pml_r=param.pml_r;
  double pml_v=param.pml_v;
  double pml_fc=param.pml_fc;
...
#define ALLOC_CPML(psi,comp,n)\
  psi.comp=new double[n];std::fill_n(psi.comp,n,0);
