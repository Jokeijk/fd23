...
void RECORD::cu_init_record(int k,PARAM &param)
{
  ...
  safecall(cudaMalloc((void**)&d_recbuf[k],sizeof(double)*lrec[k]));
  g_d_recbuf[k]=new double[lrec[k]];
...
int cpy_backhost(int deviceid,PARAM & param,FIELD & fld,FIELD & d_fld)
{
  ...
  safecall(cudaMemcpy(fld.Txx+nz1*nx,d_fld.Txx+nz1*nx,sizeof(double)*nx*tnz,cudaMemcpyDeviceToHost));
  safecall(cudaMemcpy(fld.Txz+nz1*nx,d_fld.Txz+nz1*nx,sizeof(double)*nx*tnz,cudaMemcpyDeviceToHost));
  safecall(cudaMemcpy(fld.Tzz+nz1*nx,d_fld.Tzz+nz1*nx,sizeof(double)*nx*tnz,cudaMemcpyDeviceToHost));
  safecall(cudaMemcpy(fld.U  +nz1*nx,d_fld.U  +nz1*nx,sizeof(double)*nx*tnz,cudaMemcpyDeviceToHost));
  safecall(cudaMemcpy(fld.W  +nz1*nx,d_fld.W  +nz1*nx,sizeof(double)*nx*tnz,cudaMemcpyDeviceToHost));
...
/* record */
__global__ void cudarecord_list(int nx,double* d_U,
	int nrec,int *rx, int*rz,double *rec)
{
...
void RECORD::cu_flush(int deviceid){
  if(deviceid==0){
	fid.write((char*)g_rec,sizeof(double)*nrec);
  }
}

void RECORD::cu_record(int deviceid,int nx,double* U)
{
  ...
  cudaMemcpy(g_d_recbuf[deviceid],d_recbuf[deviceid],sizeof(double)*myrec,cudaMemcpyDeviceToHost);
