...
class RECORD{
  public:
	...
	double * d_recbuf[MAXCUDAGPU];   // in GPU
	double * g_d_recbuf[MAXCUDAGPU]; // in global
	double * g_rec;                  // continous in global
	...
	void cu_record(int deviceid,int nx, double *U); 
	void cu_flush(int deviceid);

	void close();
	void toisis(PARAM & param,double shiftx,double shiftz);
