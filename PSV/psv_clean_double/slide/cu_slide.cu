...
SLIDE::SLIDE(PARAM & param)
{
  int nx=param.nx;
  int nz=param.nz;
  double before=param.before;
  double after =param.after ;
  double dt    =param.dt;
  ...
  double *buf=new double[BDIMY*nx];
  ...
  for(int k=0;k<nzblock;k+=1){
	...
	fid.read((char*)buf,sizeof(double)*BDIMY*nx);
	for(int i=0;i<nxblock;i+=1){

	  double minvalue=1E28;
	  double maxvalue=-1E28;
