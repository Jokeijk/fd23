function isis2vel_all_true_pos(filename,tcut,travtime,mag)
[b,h]=isisread(filename);
b=b(1:tcut,:);
nt=size(b,1);
dt=h.dt(1);
t=dt*((0:nt-1)+pi*pi/16);

% bt=double(b);
% bt=fftfilt(double(1./(sqrt(t))),bt); %memory too large
% b=single(bt);
% b=diff(b);
if(nargin>=4)
  M0=10^(1.5*mag+16.1-20);
else
  mag=4.0;
  M0=10^(1.5*4+16.1-20);
end
...
gain=double(h.gain(1));

mkdir([filename,'_vel/']);
if(mod(size(b,2),2)==1)
  disp('trace size error');
  return;
end

% trace(1) is at position x, which is specified in par file
% trace(2) is at position x-h

for j=1:size(b,2)/2

    i=j*2;
    
    bt=double((b(:,i)+b(:,i-1))/2);
    bt=fftfilt(double(1./(sqrt(t))),bt);
    bt=diff([0;bt]);    

    btp=double(b(:,i)-b(:,i-1))*gain;
    btp=fftfilt(double(1./(sqrt(t))),btp);
    
%     [cor,shift]=xcorr(btp,bt,3);
%     [~,k]=max(abs(cor));
%     btp(4:nt-4)=btp(4+shift(k):nt-4+shift(k));
	
	bts=sign(bt).*sqrt(abs(bt.*btp));
	...
	s.MAG=mag;
    s.O=0;
    s.DIST=(0.5*xr(i)+0.5*xr(i-1)-xs(i));
    s.GCARC=km2deg(s.DIST);
    s.NPTS=size(bt,1);
    s.B=0;
    s.E=(size(bt,1)-1)*h.dt(i);
    s.NVHDR = 6;
    s.IFTYPE = 'ITIME';
    if(nargin>=3)
        s.T8=travtime(j);
    end
    s.FILENAME=[filename,'_vel/',num2str(j,'%04d'),'.sac'];
    s.DATA1=bt/sqrt(abs(xr(i)-xs(i)))*M0;
    writesac(s);

    s.FILENAME=[filename,'_vel/p',num2str(j,'%04d'),'.sac'];
    s.DATA1=btp/sqrt(abs(xr(i)-xs(i)))*M0;
    writesac(s);

    s.FILENAME=[filename,'_vel/s',num2str(j,'%04d'),'.sac'];
    s.DATA1=bts/sqrt(abs(xr(i)-xs(i)))*M0;
    writesac(s);
end
