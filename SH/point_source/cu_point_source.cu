...
void cu_point_force(int deviceid,int it,int lsrc,float *src, PARAM &param, FIELD & fld)
{
  float temp;
  int nx=param.nx;
  int nz1=param.a_nz1[deviceid];
  ...
  ixs=xs;  izs=zs;
  if(izs>=nz1 && izs<=nz2 && it<lsrc ){ 
	safecall(cudaMemcpy(&temp,fld.V+izs*nx+ixs,sizeof(float),cudaMemcpyDeviceToHost));
	temp -= 0.5*src[it]*Mxy;
	safecall(cudaMemcpy(fld.V+izs*nx+ixs,&temp,sizeof(float),cudaMemcpyHostToDevice));
  }
